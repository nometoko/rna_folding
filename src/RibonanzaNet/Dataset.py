...
class RNA3D_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        xyz = torch.tensor(
            np.array(self.data["xyz"][idx]), dtype=torch.float32
        )  # shape: (L, 3)
        assert sequence.shape[0] == xyz.shape[0]

        # 長すぎる場合はランダムcrop
        if len(sequence) > self.max_len:
            crop_start = np.random.randint(len(sequence) - self.max_len + 1)
        ...
        if L < self.max_len:
            ...
            sequence_pad = torch.full(
                (pad_len,), self.padding_idx, dtype=sequence.dtype
            )
            sequence = torch.cat([sequence, sequence_pad], dim=0)

            # xyzのpadding
            xyz_pad = torch.full((pad_len, 3), float("nan"), dtype=xyz.dtype)
            xyz = torch.cat([xyz, xyz_pad], dim=0)

        # ここでshapeをprintしてデバッグするのもおすすめ
        # print(sequence.shape, xyz.shape)
