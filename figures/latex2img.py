...
formulas = [
    (r"\begin{align*}f(x) &= x \cdot \tanh(\mathrm{softplus}(x)) \\ \mathrm{softplus}(x) &= \log(1 + e^x) \end{align*}", "mish"),
    (r"\mathbf{e} = \left[ \left( \frac{1}{|\Omega|} \sum_{u \in \Omega} x^p_{cu} \right)^\frac{1}{p} \right]_{c = 1, \cdots, C}", "GeM"),
    (r"\begin{align*} \mathrm{Output}(Q, K, V) &= \mathrm{softmax}\left(\frac{QK^T}{\sqrt{d_k}}\right)V \\ \end{align*}", "Scaled_DotProduct_Attention")
]

output_dir = "math_figures"
os.makedirs(output_dir, exist_ok=True)

def latex_to_png_url(latex, dpi=300):
...
for formula, filename in formulas:
    ...
    # 高解像度PNGのURLを取得
    png_url = latex_to_png_url(formula, dpi=900)  # 300 DPIに設定
